      ...
      if (fullEl) {
        fullEl.innerHTML = `
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            ...
            <span class="text-[5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">DAYS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${hStr}</span>
            <span class="text-[5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">HRS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${mStr}</span>
            <span class="text-[5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">MINS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${sStr}</span>
            <span class="text-[5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">SECS</span>
          </div>
        `;
      }
    });
...
export function renderMatches() {
  ...
  filtered.forEach(t => {
    ...
    // Rule 13: Join Match button text format is strictly preserved
    const joinBtnText = `JOIN MATCH • ₹${t.entryFee}`;

    // Rule 13: Before Join = Gold, After Join = Light Green
    const joinBtnStyle = isJoined 
      ? "bg-gradient-to-r from-green-400 via-emerald-400 to-emerald-500 text-black shadow-[0_0_15px_rgba(34,197,94,0.3)] hover:shadow-[0_0_25px_rgba(34,197,94,0.5)]" 
      : "bg-gradient-to-r from-gold via-gold-light to-gold-dark text-black glow-gold-btn";

    const card = document.createElement('div');
    card.className = 'match-card-premium cursor-pointer transform hover:-translate-y-1.5 transition-all duration-300';
    card.innerHTML = `
      <!-- TOP BANNER BG SECTION (Rule 6: Reduced banner block height, Rule 7: Fully visible with very light overlay) -->
      <div class="relative h-[175px] w-full overflow-hidden shrink-0 bg-[#0c071a] rounded-t-[1.8rem]">
        ...
        >
        <!-- Rule 7: Very light overlay gradient -->
        ...
        <div class="absolute top-4 left-4 flex flex-col gap-1 z-10 text-left">
          ...
          <!-- Large Match Name (Rule 1: MATCH ID completely removed) -->
          ...
          <!-- Rule 2: Date & Time completely below the Map/Game row with 12-hour formatting -->
          <div class="flex items-center gap-2 mt-2">
            <span class="flex items-center gap-1 bg-black/60 border border-gold/15 px-2 py-0.5 rounded text-[8px] text-gold font-bold">
              📅 ${formatFriendlyDate(t.date)}
            </span>
            <span class="flex items-center gap-1 bg-black/60 border border-gold/15 px-2 py-0.5 rounded text-[8px] text-gold font-bold">
              🕒 ${format12HourTime(t.time)}
            </span>
          </div>
        </div>
        ...
        <!-- Rule 3: Floating starts in countdown aligned nicely at bottom right row of banner -->
        <div class="absolute bottom-3 right-3 z-10 bg-black/80 backdrop-blur-md px-2.5 py-1 rounded-xl border border-gold/20 text-right font-grotesk min-w-[120px] shadow-lg">
          <div class="text-[7px] text-slate-400 font-extrabold uppercase tracking-widest leading-none">STARTS IN</div>
          <div class="text-[10px] font-mono font-black text-yellow-400 mt-0.5 tracking-wider leading-none" id="mini-countdown-${t.id}">00d 00h 00m 00s</div>
        </div>
      </div>

      <!-- MAIN CARD CONTENT BODY (Rule 6: Reduced Match Card container spacing slightly) -->
      <div class="p-3.5 space-y-2.5 bg-[#05020c]/95 border-t border-[#8b5cf6]/10">

        <!-- Rule 8: Keep four info boxes in this exact order: Prize Pool, Entry Fee, Per Kill, Winner Prize -->
        ...
        <!-- Rule 9: Keep Player Section (Joined, Remaining Slots, Status Badge, Progress Bar) -->
        ...
        <!-- Rule 10: Keep bottom countdown exactly as it is (Rule 4 & 5: Improved premium centered STARTS AT display with 12-hour formatting) -->
        <div class="bg-black/40 border border-[#8b5cf6]/20 rounded-2xl p-2.5 flex items-center justify-between gap-3 text-slate-300">
          <div class="flex flex-col gap-1 text-left">
            <div class="flex items-center gap-1 text-[8.5px] font-extrabold text-[#c084fc] uppercase tracking-wider font-grotesk leading-none">
              <i class="fa-regular fa-clock"></i>
              <span>MATCH STARTS IN</span>
            </div>
            <!-- Countdown grid with 4 digit boxes -->
            <div class="flex items-center gap-1 mt-0.5 font-mono text-center" id="full-countdown-${t.id}">
              <div class="bg-black/60 border border-purple-500/10 rounded px-1 py-0.5 min-w-[28px]">
                <span class="block text-[10px] font-bold text-white leading-none">00</span>
                <span class="text-[4.5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">DAYS</span>
              </div>
              <div class="bg-black/60 border border-purple-500/10 rounded px-1 py-0.5 min-w-[28px]">
                <span class="block text-[10px] font-bold text-white leading-none">00</span>
                <span class="text-[4.5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">HRS</span>
              </div>
              <div class="bg-black/60 border border-purple-500/10 rounded px-1 py-0.5 min-w-[28px]">
                <span class="block text-[10px] font-bold text-white leading-none">00</span>
                <span class="text-[4.5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">MINS</span>
              </div>
              <div class="bg-black/60 border border-purple-500/10 rounded px-1 py-0.5 min-w-[28px]">
                <span class="block text-[10px] font-bold text-white leading-none">00</span>
                <span class="text-[4.5px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">SECS</span>
              </div>
            </div>
          </div>
          
          <!-- Rule 4: Improved Premium center-aligned STARTS AT display with proper 12-hour clean typography -->
          <div class="text-center space-y-1 border-l border-[#8b5cf6]/20 pl-4 min-w-[110px]">
            <span class="text-[8px] text-gold font-extrabold uppercase tracking-widest block font-grotesk leading-none">
              STARTS AT
            </span>
            <div class="space-y-0.5 text-[9px] font-grotesk">
              <strong class="block text-white font-extrabold leading-tight">📅 ${formatFriendlyDate(t.date)}</strong>
              <span class="block text-yellow-500 font-mono font-bold leading-none">${format12HourTime(t.time)}</span>
            </div>
          </div>
        </div>

        <!-- Rule 11: Keep compact Room Credentials Copy ID & Password details layout -->
        ...
        <!-- Rule 12: View Details button (exact same layout & functions) & Rule 13: Join button layouts & states -->
        <div class="grid grid-cols-2 gap-2.5 pt-0.5">
          ...
          <button class="py-2 px-2.5 ${joinBtnStyle} rounded-xl font-rajdhani font-black text-xs uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all join-match-btn">
            ${joinBtnText}
          </button>
        </div>
